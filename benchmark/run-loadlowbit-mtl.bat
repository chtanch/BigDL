...
@REM save files
copy run-loadlowbit-mtl.bat "%OUTPUT_DIR%"
pip list > "%OUTPUT_DIR%\requirements.txt"

@REM Run benchmarks
@REM transformers==4.34.0
python -m pip install transformers==4.34.0
copy config_loadlowbit_434.yaml ..\python\llm\dev\benchmark\all-in-one\config.yaml
copy config_loadlowbit_434.yaml "%OUTPUT_DIR%"
cd ..\python\llm\dev\benchmark\all-in-one
python run.py
move *.csv "%OUTPUT_DIR%"
cd %CWD%
...
copy config_loadlowbit_438.yaml ..\python\llm\dev\benchmark\all-in-one\config.yaml
copy config_loadlowbit_438.yaml "%OUTPUT_DIR%"
cd ..\python\llm\dev\benchmark\all-in-one
python run.py
move *.csv "%OUTPUT_DIR%"
cd %CWD%
...
copy config_loadlowbit_439.yaml ..\python\llm\dev\benchmark\all-in-one\config.yaml
copy config_loadlowbit_439.yaml "%OUTPUT_DIR%"
cd ..\python\llm\dev\benchmark\all-in-one
python run.py
move *.csv "%OUTPUT_DIR%"
cd %CWD%

@REM transformers==4.40.0
python -m pip install transformers==4.40.0
copy config_loadlowbit_440.yaml ..\python\llm\dev\benchmark\all-in-one\config.yaml
copy config_loadlowbit_440.yaml "%OUTPUT_DIR%"
cd ..\python\llm\dev\benchmark\all-in-one
python run.py
move *.csv "%OUTPUT_DIR%"
cd %CWD%
